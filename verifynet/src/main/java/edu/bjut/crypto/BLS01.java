...
public class BLS01 {
    ...
    public byte[] encrypt(String message, CipherParameters privateKey) {
        byte[] bytes = message.getBytes();

        BLS01Signer signer = new BLS01Signer(new PlainDigest());
        signer.init(true, privateKey);
        signer.update(bytes, 0, bytes.length);
    ...
    public boolean decrypt(byte[] ciphertxt, String message, CipherParameters publicKey) {
        byte[] bytes = message.getBytes();
        ...
        return signer.verifySignature(ciphertxt);
    }
